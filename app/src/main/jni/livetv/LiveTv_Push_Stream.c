#include "com_jcy_ndk_nativeC_liveTv_PushNative.h"
...
#include <pthread.h>
#include <livetv/x264/include/x264.h>
#include <livetv/faac/include/faac.h>
#include <malloc.h>
#include <livetv/rtmpdump/include/rtmp.h>
#include "queueComm.h"
...
//获取JavaVM
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *reserved) { // JNI_OnLoad 同一工程只能有一处使用此函数
    javaVM = vm;
...
JNIEXPORT void JNICALL Java_com_jcy_ndk_nativeC_liveTv_PushNative_setAudioOptions
    ...
    audio_encode_handle = faacEncOpen((unsigned long) sampleRateInHz, (unsigned int) numChannels,
...
JNIEXPORT void JNICALL Java_com_jcy_ndk_nativeC_liveTv_PushNative_fireAudio
    ...
    while (nByteCount < nBufferSize) {
        int audioLength = (int) nInputSamples;
